...
class ModelArguments:
    model_name_or_path: Optional[str] = field(
        default="/data/42-julia-hpc-rz-lsx/s386162/models/iter-1250000-ckpt"
    )
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    output_dir: str = field(default='/data/42-julia-hpc-rz-lsx/s386162/out/models/tinyllama_1b', metadata={"help": 'The output dir for logs and checkpoints'})
    ...
    max_steps: int = field(default=51759, metadata={"help": 'How many optimizer update steps to take'})
...
ALPACA_PROMPT_DICT = {
    "prompt_input": (
        "Unten steht eine Anweisung, die eine Aufgabe beschreibt, gepaart mit einer Eingabe, die weiteren Kontext liefert. "
        "Schreiben Sie eine Antwort, die die Aufgabe angemessen erfüllt.\n\n"
        "### Anleitung:\n{instruction}\n\n### Eingabe:\n{input}\n\n### Antwort: "
    ),
    "prompt_no_input": (
        "Unten steht eine Anweisung, die eine Aufgabe beschreibt. "
        "Schreiben Sie eine Antwort, die die Aufgabe angemessen erfüllt.\n\n"
        "### Anleitung:\n{instruction}\n\n### Antwort: "
    ),
}
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        if dataset_name == 'zicklein':
            return load_dataset("/home/s386162/finetune/data")
    ...
    def format_dataset(dataset, dataset_format):
        if (
            ...
            (dataset_format is None and args.dataset in ['alpaca', 'alpaca-clean', 'zicklein'])
        ):
    ...
    # Split train/eval_data, reduce size
    if args.do_eval or args.do_predict:
        if 'eval_data' in dataset:
...
def train():
    ...
    if args.do_eval:
        ...
        metrics = trainer.evaluate(metric_key_prefix="eval_data")
        trainer.log_metrics("eval_data", metrics)
        trainer.save_metrics("eval_data", metrics)
        all_metrics.update(metrics)
    # Prediction
    if args.do_predict:
        logger.info("*** Predict ***")
